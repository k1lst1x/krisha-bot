...
def test_normalize_phone_variants() -> None:
    assert normalize_phone("+7 (776) 227-86-84") == "+77762278684"
    assert normalize_phone("87762278684") == "+77762278684"
    assert normalize_phone("7762278684") == "+77762278684"
...
def test_access_store_authorizes_only_allowed_phone(tmp_path: Path) -> None:
    ...
    store.seed_allowed_phones(["+77762278684"], source="test")
    assert store.is_phone_allowed("+7 776 227 86 84")
    assert not store.is_phone_allowed("+7 700 000 00 00")

    store.authorize_user(
        telegram_user_id=1234,
        chat_id="9876",
        phone="+7 776 227 86 84",
        username="demo",
        first_name="Demo",
    )
...
def test_access_store_replace_allowed_phones_removes_old_env_entries(tmp_path: Path) -> None:
    ...
    assert not store.is_phone_allowed("+7 776 227 86 84")
